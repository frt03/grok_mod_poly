...
def main(config):
    # project = 'ssl_grok_transfer_finetune'
    project = 'poly_grok_finetune'
    ...
    if config.model == 'transformer':
        ...
        pretrained = torch.load(config.ckpt_path, map_location=torch.device('cpu'))['model']
        if config.load_weigths[0] == 'full':
            model.load_state_dict(pretrained)
        else:
            del_key = []
            if 'unembed' not in config.load_weigths:
                del_key.extend(unembed_keys)
            if 'embed' not in config.load_weigths:
                del_key.extend(embed_keys)
            if 'transformer' not in config.load_weigths:
                del_key.extend(transformer_keys)
            for k in del_key:
                del pretrained[k]
            model.load_state_dict(pretrained, strict=False)
        # freeze some parameters
        if config.frozen_weigths[0] == 'full':
            for _, param in model.named_parameters():
                param.requires_grad = False
        else:
            frozen_key = []
            if 'unembed' in config.frozen_weigths:
                frozen_key.extend(unembed_keys)
            if 'embed' in config.frozen_weigths:
                frozen_key.extend(embed_keys)
            if 'transformer' in config.frozen_weigths:
                frozen_key.extend(transformer_keys)
            for name, param in model.named_parameters():
                if name in frozen_key:
                    param.requires_grad = False
...
if __name__ == '__main__':
    # from config.config_add_transformer_emb_ft import Exp
    ...
    parser.add_argument('--ckpt_name', type=str, default='pt_add')
    parser.add_argument('--config', type=str, default='config.config_add_transformer_emb_ft')
    ...
    config.exp_name = f'{config.fn_name}_{config.model}_{config.num_layers}L_{config.d_model}D_{config.p}P_{config.frac_train}F_{config.lr}LR_{config.weight_decay}WD_{config.is_symmetric_input}S_{config.weight_scale}WS_{config.lp}LP_{config.lp_alpha}LPA_emb_rm_eq_ft_{config.identifier}_seed_{config.seed}'
