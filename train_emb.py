...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='config.config_add_transformer_emb')
    ...
    config.exp_name = f'{config.fn_name}_{config.model}_{config.num_layers}L_{config.d_model}D_{config.p}P_{config.frac_train}F_{config.lr}LR_{config.weight_decay}WD_{config.is_symmetric_input}S_{config.weight_scale}WS_{config.lp}LP_{config.lp_alpha}LPA_emb_rm_eq_seed_{config.seed}'
    # {config.weight_scale}WS_{config.lp}LP_{config.lp_alpha}LPA_emb_seed_{config.seed}'
